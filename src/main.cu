...
int run_frodo() {
    ...
    if (op == 1) {
        std::vector<uint8_t> pk(PQCUDA_FRODO976_PUBLIC_KEY_BYTES);
        std::vector<uint8_t> sk(PQCUDA_FRODO976_SECRET_KEY_BYTES);
        if (pqcuda_frodo_keypair(pk.data(), sk.data()) != 0) return 1;
        if (!write_hex_file("frodo976_public_key.hex", pk.data(), pk.size()) ||
            !write_hex_file("frodo976_private_key.hex", sk.data(), sk.size())) {
            std::cerr << "KeyGen 결과 파일 저장에 실패했습니다.\n";
            return 1;
        }
        std::cout << "Public key (" << pk.size()
                  << " bytes): frodo976_public_key.hex\n"
                  << "Private key (" << sk.size()
                  << " bytes): frodo976_private_key.hex\n";
        return 0;
    }
    if (op == 2) {
        std::vector<uint8_t> pk, ct(PQCUDA_FRODO976_CIPHERTEXT_BYTES);
        std::vector<uint8_t> ss(PQCUDA_FRODO976_SHARED_SECRET_BYTES);
        if (!read_hex("Public key", PQCUDA_FRODO976_PUBLIC_KEY_BYTES, pk)) return 2;
        if (pqcuda_frodo_encapsulate(ct.data(), ss.data(), pk.data()) != 0) return 1;
        if (write_hex_file("frodo976_ciphertext.hex", ct.data(), ct.size()) &&
            write_hex_file("frodo976_shared_key_encap.hex",
                           ss.data(), ss.size()))
            std::cout << "Ciphertext (" << ct.size()
                      << " bytes): frodo976_ciphertext.hex\n"
                      << "Shared secret (" << ss.size()
                      << " bytes): frodo976_shared_key_encap.hex\n";
        else
            std::cerr << "Encapsulation 결과 파일 저장에 실패했습니다.\n";
        return 0;
    }
    if (op == 3) {
        std::vector<uint8_t> ct, sk, ss(PQCUDA_FRODO976_SHARED_SECRET_BYTES);
        if (!read_hex("Ciphertext", PQCUDA_FRODO976_CIPHERTEXT_BYTES, ct) ||
            !read_hex("Private key", PQCUDA_FRODO976_SECRET_KEY_BYTES, sk)) return 2;
        if (pqcuda_frodo_decapsulate(ss.data(), ct.data(), sk.data()) != 0) return 1;
        print_hex("Shared secret", ss.data(), ss.size());
        if (write_hex_file("frodo976_shared_key_decap.hex",
                           ss.data(), ss.size()))
            std::cout << "결과를 frodo976_shared_key_decap.hex에 "
                         "저장했습니다.\n";
        else
...
int main(int argc, char **argv) {
    ...
    if (algorithm == 1) return run_frodo();
