...
const int kernel_variant_counts[DILITHIUM_TUNE_STAGE_COUNT] = {2, 2, 3, 2, 2, 2, 2};
...
{
    if (!valid_tuning_stage(stage) || variant < 0 || variant >= kernel_variant_counts[stage]) return nullptr;
    switch (stage) {
    case DILITHIUM_TUNE_SHAKE:
        return variant == 0 ? "notmp_shake_kernel" : "notmp_shake_new_kernel";
    case DILITHIUM_TUNE_MATRIX_EXPAND:
        return variant == 0 ? "polyvec_matrix_expand_kernel" : "polyvec_matrix_expand_opt_kernel";
    case DILITHIUM_TUNE_UNPACK_NTT:
        if (variant == 0) return "unpack_fuse_ntt_kernel";
        return variant == 1 ? "unpack_fuse_ntt_radix2_kernel" : "unpack_fuse_ntt_radix2_opt_kernel";
    case DILITHIUM_TUNE_COMPUTE_Y:
        return variant == 0 ? "compute_y_kernel" : "compute_y_opt_kernel";
    case DILITHIUM_TUNE_COMPUTE_W:
        return variant == 0 ? "compute_w_32t_kernel" : "compute_w_128t_kernel";
    case DILITHIUM_TUNE_COMPUTE_CP:
        return variant == 0 ? "compute_cp_kernel" : "compute_cp_opt_kernel";
    case DILITHIUM_TUNE_REJECTION:
        return variant == 0 ? "rej_loop_32t_kernel" : "rej_loop_128t_kernel";
    default:
        return nullptr;
    }
}
...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk, uint8_t *d_keypair_mem_pool, size_t keypair_mem_pool_pitch, size_t batch_size, cudaStream_t stream, size_t rand_index) 
{
    ...
    gpu_keypair<<<batch_size, 32, 0, stream>>>(d_pk_rho, d_pk_t1, d_sk_rho, d_sk_key, d_sk_tr, d_sk_s1_packed, d_sk_s2_packed, d_sk_t0_packed, d_t1, keypair_mem_pool_pitch, rand_index);
...
//    timer_rej_loop.start();
    ...
    {
        notmp_shake_kernel<SHAKE256_RATE><<<batch_size, dim3(32, 1), SHAKE256_RATE, stream>>>(d_mu, d_sign_mem_pool_pitch, CRHBYTES, d_tr, d_sign_mem_pool_pitch, SEEDBYTES + mlen, batch_size);
    } else {
        notmp_shake_new_kernel<SHAKE256_RATE, 0x1f><<<(batch_size + 3) / 4, dim3(32, 4), SHAKE256_RATE, stream>>>(d_mu, d_sign_mem_pool_pitch, CRHBYTES, d_tr, d_sign_mem_pool_pitch, SEEDBYTES + mlen, batch_size);
    }
    ...
    if (selected_kernel_variants[DILITHIUM_TUNE_SHAKE] == 0) 
    {
        notmp_shake_kernel<SHAKE256_RATE><<<batch_size, dim3(32, 1), SHAKE256_RATE, stream>>>(d_rhoprime, d_sign_mem_pool_pitch, CRHBYTES, d_key, d_sign_mem_pool_pitch, SEEDBYTES + CRHBYTES, batch_size);
    } else {
        notmp_shake_new_kernel<SHAKE256_RATE, 0x1f><<<(batch_size + 3) / 4, dim3(32, 4), SHAKE256_RATE, stream>>>(d_rhoprime, d_sign_mem_pool_pitch, CRHBYTES, d_key, d_sign_mem_pool_pitch, SEEDBYTES + CRHBYTES, batch_size);
    }
    tuning_stop(DILITHIUM_TUNE_SHAKE, stream);
#endif
    ...
    {
        unpack_fuse_ntt_radix2_kernel<<<batch_size, 128, 0, stream>>>(d_s1, d_s2, d_t0, d_sk_s1_packed, d_sk_s2_packed, d_sk_t0_packed, d_sign_mem_pool_pitch);
    } else 
    {
        unpack_fuse_ntt_radix2_opt_kernel<<<batch_size, 128, 0, stream>>>(d_s1, d_s2, d_t0, d_sk_s1_packed, d_sk_s2_packed, d_sk_t0_packed, d_sign_mem_pool_pitch);
    }
...
//        timer_y.start();
        ...
        {
            compute_y_kernel<<<exec_threshold, 32, 0, stream>>>(d_y, d_rhoprime, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        } else 
        {
            compute_y_opt_kernel<<<(exec_threshold + 3) / 4, dim3(32, 4), 0, stream>>>(d_y, d_rhoprime, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch, exec_threshold);
        }
...
//        timer_w.start();
        ...
        {
            compute_w_32t_kernel<<<exec_threshold, 32, 0, stream>>>(d_z, d_w0, d_w1, d_w1_packed, d_y, d_mat, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        } else 
        {
            compute_w_128t_kernel<<<exec_threshold, 128, 0, stream>>>(d_w0, d_w1, d_w1_packed, d_y, d_mat, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        }
...
//        timer_cp.start();
        ...
        {
            compute_cp_kernel<<<exec_threshold, 32, 0, stream>>>(d_cp, d_mu, d_seed, d_mu, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        } else 
        {
            compute_cp_opt_kernel<<<(exec_threshold + 3) / 4, dim3(32, 4), 0, stream>>>(d_cp, d_mu, d_seed, d_mu, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch, exec_threshold);
        }
...
//        timer_rej.start();
        ...
        {
            rej_loop_32t_kernel<<<exec_threshold, 32, 0, stream>>>(d_y, d_z, d_w0, d_w1, d_cp, d_z_packed, d_hint, lut.d_done_lut, d_s1, d_s2, d_t0, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        } else 
        {
            rej_loop_128t_kernel<<<exec_threshold, 128, 0, stream>>>(d_y, d_z, d_w0, d_w1, d_cp, d_z_packed, d_hint, lut.d_done_lut, d_s1, d_s2, d_t0, lut.d_exec_lut, d_sign_mem_pool_pitch, d_sign_mem_pool_pitch);
        }
...
//        timer_rej.stop();
    ...
    } else 
    {
        ...
        if (selected_kernel_variants[DILITHIUM_TUNE_COMPUTE_Y] == 0) 
        {
            compute_y_kernel<<<exec_threshold, 32, 0, stream>>>(d_temp_y, d_rhoprime, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        } else 
        {
            compute_y_opt_kernel<<<(exec_threshold + 3) / 4, dim3(32, 4), 0, stream>>>(d_temp_y, d_rhoprime, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch, exec_threshold);
        }
        tuning_stop(DILITHIUM_TUNE_COMPUTE_Y, stream);
        ...
        if (selected_kernel_variants[DILITHIUM_TUNE_COMPUTE_W] == 0) 
        {
            compute_w_32t_kernel<<<exec_threshold, 32, 0, stream>>>(d_temp_z, d_temp_w0, d_temp_w1, d_temp_w1_packed, d_temp_y, d_mat, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        } else 
        {
            compute_w_128t_kernel<<<exec_threshold, 128, 0, stream>>>(d_temp_w0, d_temp_w1, d_temp_w1_packed, d_temp_y, d_mat, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        }
        tuning_stop(DILITHIUM_TUNE_COMPUTE_W, stream);
        ...
        if (selected_kernel_variants[DILITHIUM_TUNE_COMPUTE_CP] == 0) 
        {
            compute_cp_kernel<<<exec_threshold, 32, 0, stream>>>(d_temp_cp, d_temp_mu, d_temp_seed, d_mu, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        } else 
        {
            compute_cp_opt_kernel<<<(exec_threshold + 3) / 4, dim3(32, 4), 0, stream>>>(d_temp_cp, d_temp_mu, d_temp_seed, d_mu, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch, exec_threshold);
        }
        tuning_stop(DILITHIUM_TUNE_COMPUTE_CP, stream);
        ...
        if (selected_kernel_variants[DILITHIUM_TUNE_REJECTION] == 0) 
        {
            rej_loop_32t_kernel<<<exec_threshold, 32, 0, stream>>>(d_temp_y, d_temp_z, d_temp_w0, d_temp_w1, d_temp_cp, d_temp_z_packed, d_temp_hint, lut.d_done_lut, d_s1, d_s2, d_t0, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        } else 
        {
            rej_loop_128t_kernel<<<exec_threshold, 128, 0, stream>>>(d_temp_y, d_temp_z, d_temp_w0, d_temp_w1, d_temp_cp, d_temp_z_packed, d_temp_hint, lut.d_done_lut, d_s1, d_s2, d_t0, lut.d_exec_lut, d_sign_mem_pool_pitch, d_temp_mem_pool_pitch);
        }
        tuning_stop(DILITHIUM_TUNE_REJECTION, stream);
        ...
        // launch copy kernel
        sig_copy_kernel<<<copy_count, 96, 0, stream>>>(d_sig, d_sign_mem_pool_pitch, d_temp_sig, d_temp_mem_pool_pitch, lut.d_copy_lut);
...
void crypto_sign_verify(int *ret, const uint8_t *sig, size_t sig_pitch, size_t siglen, const uint8_t *m, size_t m_pitch, size_t mlen, const uint8_t *pk, uint8_t *d_verify_mem_pool, size_t verify_mem_pool_pitch, size_t batch_size, cudaStream_t stream)                         
{
    ...
    gpu_verify<<<batch_size, 32, 0, stream>>>(d_ret, d_c, d_z_packed, d_h_packed, d_rho, d_t1_packed, d_muprime, d_m, d_mu, d_w1prime_packed, d_ctilde, d_mat, d_z, d_t1, d_w1prime, d_h, d_cp, mlen, verify_mem_pool_pitch);
