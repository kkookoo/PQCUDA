 ...
 * by Gilles Van Assche, Daniel J. Bernstein, and Peter Schwabe
   Adapted from Crystals Kyber Reference Codebase and Parallelized by Arpan Jati
 */
 ...
 **************************************************/
/*__device__  uint64_t load64(unsigned char* x)
{
...
__device__   uint64_t pqcuda_thread_KeccakF_RoundConstants[NROUNDS] =
{
...
/*__device__ void KeccakF1600_StatePermute(uint64_t* state)
{
	...
	for (round = 0; round < NROUNDS; round += 2)
	{
		...
		BCu = ROL(Asu, 14);
		Eba = BCa ^ ((~BCe) & BCi);
		Eba ^= (uint64_t)pqcuda_thread_KeccakF_RoundConstants[round];
		Ebe = BCe ^ ((~BCi) & BCo);
		Ebi = BCi ^ ((~BCo) & BCu);
		Ebo = BCo ^ ((~BCu) & BCa);
		Ebu = BCu ^ ((~BCa) & BCe);
		...
		BCu = ROL(Esu, 14);
		Aba = BCa ^ ((~BCe) & BCi);
		Aba ^= (uint64_t)pqcuda_thread_KeccakF_RoundConstants[round + 1];
		Abe = BCe ^ ((~BCi) & BCo);
		Abi = BCi ^ ((~BCo) & BCu);
		Abo = BCo ^ ((~BCu) & BCa);
		Abu = BCu ^ ((~BCa) & BCe);
...
__device__ void KeccakF1600_StatePermute(uint64_t* st_io)
{
	...
	for (r = 0; r < NROUNDS; r++) {
		...
		//  Iota
		st[0] ^= pqcuda_thread_KeccakF_RoundConstants[r];
	}
...
*              - unsigned int r:                 rate in bytes (e.g., 168 for SHAKE128)
**************************************************/
__device__  void keccak_squeezeblocks(unsigned char* h,
...
**************************************************/
__device__ void shake128_absorb(uint64_t* s,
	unsigned char* input,
	...
	unsigned long long inlen)
{
	...
	if (X < COUNT)
	{
		...
		keccak_absorb(s, SHA3_256_RATE, input + o_input, inlen, 0x06);
	...
	unsigned long long inlen)
{
	...
	if (X < COUNT)
	{
		...
		keccak_absorb(s, SHA3_512_RATE, (input + o_input), inlen, 0x06);
